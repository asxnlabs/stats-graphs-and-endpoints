...
const CodeRewardsChart = ({ data }) => {
    ...
    const processChartData = () => {
        if (!data?.claim_metrics?.code) return {
            totalData: [],
            claimedUnclaimedData: [],
            unclaimedBreakdownData: []
        };

        const codeData = data.claim_metrics.code;
        const totalEmissions = codeData.total_code_emissions;
        const claimed = codeData.claimed_code_rewards;
        const unclaimed = codeData.unclaimed_code_emissions;
        const stakedUnclaimed = codeData.total_code_staked_reward_sum;
        const unclaimedNotStaked = unclaimed - stakedUnclaimed;

        // Total Emissions Ring
        const totalData = [{
            name: "Total Code Emissions",
    ...
    return (
        <div className="supply_chart_background" style={{ 
            ...
            <p className="chart-note" style={{ 
                ...
                Distribution of Total MOR Emissions
            </p>
            <div style={{ 
    ...
    alignItems: 'center',
    gap: '20px',
    justifyContent: 'flex-start', // Add this to align content to the left
    paddingLeft: '0px' // Add some padding from the left edge
}}>
    <div style={{ 
        width: isMobile ? '100%' : '60%', // Reduce from 75% to 60%
        height: isMobile ? '60%' : '100%',
        display: 'flex',
        justifyContent: 'flex-start' // Add this to align chart to the left
    }}>
        <ResponsiveContainer>
...
>
    {unclaimedBreakdownData.map((entry, index) => (
        ...
        />
                ...
                <div style={{ 
        width: isMobile ? '100%' : '25%', // Reduce from 25% to 30%
        height: isMobile ? '40%' : '100%',
        display: 'flex',
        alignItems: 'center',
        paddingLeft: '0' // Remove legend padding
    }}>
                    <CustomLegend 
                        payload={legendData}
...
export default CodeRewardsChart;